...
构建产物位于 `dist-extension/`。在 Chrome 的“加载已解压的扩展程序”中选择该目录即可。扩展包含 popup、side panel、background service worker、offscreen recorder 和 content script，快捷键默认为 `Ctrl+Shift+Y`，macOS 为 `Command+Shift+Y`。
