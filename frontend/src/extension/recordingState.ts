...
export function appendFinal(state: ExtensionRecordingState, rawText: string): ExtensionRecordingState {
  return {
    ...state,
    partial: "",
    finals: [...state.finals, { id: nextFinalId++, rawText, polishedText: null }],
  };
}

export function applyPolished(
  state: ExtensionRecordingState,
  polishedText: string,
): ExtensionRecordingState {
  const index = state.finals.findIndex((sentence) => !sentence.polishedText);
  if (index >= 0) {
    const finals = state.finals.slice();
    finals[index] = { ...finals[index], polishedText };
    return { ...state, finals };
  }
  return {
    ...state,
    finals: [...state.finals, { id: nextFinalId++, rawText: "", polishedText }],
  };
}
